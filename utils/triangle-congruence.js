$.extend(KhanUtil, {
	...
	initCongruence_new: function(options) {
        options = $.extend({
            ticks: [],
            numArcs: [0, 0, 0],
            reflected: false
        ...
        if (options.type === "SSS") {
            ...
            options.angles[2] = randomAngle(options.angles[2]);
            options.ticks = [1, 2, 3];
            var triangle = KhanUtil.addInteractiveTriangle_new(options);
        ...
        KhanUtil.currentGraph.interactiveTriangle_new = triangle;

    },

	addInteractiveTriangle_new: function(options) {
        var triangle = $.extend({
            points: [],
            lines: [],
        ...
        triangle.update = function() {
            ...
            for (var angle = 0; angle < 2; ++angle) {
                ...
                triangle.arcs[angle] = KhanUtil.currentGraph.drawArcs(triangle.points[angle].coord, triangle.points[angle + 1].coord, triangle.points[angle + 2].coord, options.numArcs[angle]);
            }
            if (options.numArcs[2]) {
                $(triangle.arcs[2]).each(function() { this.remove(); });
                triangle.arcs[angle] = KhanUtil.currentGraph.drawArcs(triangle.points[2].coord, triangle.points[3].coord, triangle.points[1].coord, options.numArcs[2]);
            }
        ...
        var coord = [0, 0];
        triangle.points.push(KhanUtil.currentGraph.addMovablePoint({ coord: coord }));
        ...
        triangle.lines.push(KhanUtil.currentGraph.addMovableLineSegment({ pointA: triangle.points[0], pointZ: triangle.points[1], ticks: options.ticks[0], highlightStyle: { "stroke": KhanUtil.BLUE, "stroke-width": 4 } }));
        triangle.lines.push(KhanUtil.currentGraph.addMovableLineSegment({ pointA: triangle.points[1], pointZ: triangle.points[2], ticks: options.ticks[1], highlightStyle: { "stroke": KhanUtil.BLUE, "stroke-width": 4 } }));
        triangle.lines.push(KhanUtil.currentGraph.addMovableLineSegment({ pointA: triangle.points[2], pointZ: triangle.points[3], ticks: options.ticks[2], highlightStyle: { "stroke": KhanUtil.BLUE, "stroke-width": 4 } }));

        triangle.rotationPoint = KhanUtil.currentGraph.addMovablePoint({ visible: false });
    ...
    addInteractiveTriangle: function(options) {
        ...
        $(".question").prepend("<button id=\"reflect\">旋轉圖形</button>");
        $("button#reflect").bind("click", function(event) {
            this.blur();
            if (!triangle.animating) {
                triangle.animating = true;
